...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--color-by-saros",
        action="store_true",
        help="Assign a unique color to each Saros series instead of encoding years since.",
    )
    parser.add_argument(
        "--saros-opaque",
        action="store_true",
...
def main() -> None:
    ...
    add_world_outline_trace(figure)
    descriptor_lines = []
    base_title = "Time Since Total Solar Eclipse"
    if args.color_by_saros:
        base_title = "Saros Series Coverage (History)"
    ...
    title_lines = [base_title, *descriptor_lines]
    if cutoff_label:
